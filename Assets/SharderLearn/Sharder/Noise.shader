	...
	Properties
	{
		...
		SubShader
		{
			...
			CGPROGRAM
				...
				{
					float speed = _Time.y * 0.1;

					float2 noiseUV1 = (0.2 * uv + speed * float2(0.0,0.5)) * 80.0;


					float2 noiseUV2 = (0.2 * uv + speed * float2(0.0,1)) * 80.0;

					float noiseColor1 = noise(noiseUV1);
					float noiseColor2 = noise(noiseUV2);

					float randomValue = noiseColor1 + noiseColor2;
